...
// delay time between controls
unsigned long droptime=300;
 ...
 *  6 -- T
*/
// This function rotates the current block clockwise
int rotate(int block, int type, int x, int y){
  switch(block){
    // I block rotate
    case 0:
      switch(type){
        case 0:
          // block too low, rotate closely to the ground y = 3
          if (y - 3 < 0) {
            return 2;
          }
          return 1;
          break;
        case 1:
          // block too right, rotate closely to the right side wall x  = 4
          if (x + 3  > 7){
            return 3;
          }
          return 0;
...
void loop(){
  ...
  if (SW_state == 1 | new_block == 1) {
    if (new_block == 1){
      ...
      delay(droptime);
      rotate_flag = rotate(block, type, live_x, live_y);
      switch(block){
        // I block
        case 0:
          switch(rotate_flag){
            case 0:
              sel_block(block, type, live_x, live_y, false);
              type = rotate_flag;
              sel_block(block, type, live_x, live_y, true);
              break;
            case 1:
